...
const Main = () => {
  ...
  const fetchBoxes = async () => {
    try {
      ...
      const { data } = await axios.get('http://13.125.98.73:3000/api/posts/main');
      setBoxes(data.posts);
    } catch (error) {
      console.error(error);
    }
  ...
  const { data } = useQuery('getMainList', async () => {
    ...
    const mainList = await axios.get(`http://13.125.98.73:3000/api/posts/main`);
  ...
  const openDetail = async (postId) => {
    try {
      ...
      const response = await axios.get(`http://13.125.98.73:3000/api/posts/${postId}`);
